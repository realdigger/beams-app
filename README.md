...
светильниками моделей **BEAMS 2 PRO R-8**, **BEAMS 2 MAX R-10**,
**BEAMS 2 PRO F-6**, **BEAMS 2 MAX F-8** и **BEAMS MAX F-8**.

При отправке сообщений об ошибках обязательно указывайте точную модель
Android-устройства, на котором запущено приложение, и прилагайте техническую
информацию светильника. Её можно скопировать во встроенном интерфейсе
светильника по нажатию на надпись TrueSpectrum внизу страницы или в приложении
в разделе «Устройство» по кнопке «Техническая информация», если удалось
подключиться к светильнику. Если доступно создание отчёта для диагностики,
прикладывайте также этот JSON-файл.
...
- Создание JSON-отчёта для диагностики: в нём сохраняются показания
  приложения, краткая техническая информация и ответы известных API
  светильника. IP-адрес и имя хоста в отчёт не включаются.
...
сети. Для обычной работы не требуются облачная учётная запись или внешние
